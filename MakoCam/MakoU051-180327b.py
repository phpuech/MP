...
#        frame.waitFrameCapture()#was set to a 1000
        ...
        stockageimage.append(image)
        stockagetemps.append(temps)
        stockageframe.append(framecount)
        
        test=len(stockageimage)
...
while j < l:
    name=repertoire+nom+'-'+str(stockageframe[j])+'.tiff'
    cv2.imwrite(name,stockageimage[j])
    cv2.imshow("Test",stockageimage[j])
    file.write(str(stockageframe[j])+' '+str(stockagetemps[j])+'\n')
    j+=1
file.close()
