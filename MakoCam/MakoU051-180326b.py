...
exposure=20 #msec BE CAREFUL : 1 or 2 msec (3 does not work for fps 380)
showevery=1 #frames : fps/10 au min, 100 c'est mieux
...
with Vimba() as vimba:
    ...
    blanc= blanck(frame.height,frame.width,1)
    noir= blanck2(frame.height,frame.width,1)

    while 1:

        #frame.waitFrameCapture()#was set to a 1000
        tempsimage=c0.Timestamp
        txtframecount=str(framecount)
        if framecount==0:
...
#            stockageram[txtframecount]=np.ndarray(buffer=frame_data,
#               	             dtype=np.uint8,
#                	             shape=(frame.height,frame.width,1))
            if framecount%2 == 0:
                stockageram[txtframecount]=blanc
            else:
                stockageram[txtframecount]=noir
            if framecount % showevery == 0:
                cv2.imshow("Type ESC to exit",stockageram[txtframecount])
                k = cv2.waitKey(1)
            framecount+=1
        except:
        ...
        #frame_data = frame.getBufferByteData()
#        if success:
#            stockageram[txttemps]=np.ndarray(buffer=frame_data,
#                	             dtype=np.uint8,
#                	             shape=(frame.height,frame.width,1))
#            if framecount % showevery == 0:
#                cv2.imshow("Type ESC to exit",stockageram[txttemps])
#                k = cv2.waitKey(1)
#        else:
#            stockageram[txttemps]=blanck(frame.height,frame.width,1)
