...
#            image2=black#np.array([[255,255],[255,255]])
    ...
    while j<l:
        ...
        framedata = stockageram[j].getBufferByteData()
        image2=np.ndarray(buffer=framedata,
                	             dtype=np.uint8,
                	             shape=(frame.height,frame.width,1))
        cv2.putText(image2,str(imagetemps[j])+' msec', (int(25), int(50)),cv2.FONT_HERSHEY_SIMPLEX, 1, 255,2, cv2.LINE_AA)
