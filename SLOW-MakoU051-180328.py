...
with Vimba() as vimba:
    ...
    while 1:
        ...
        if success:
            ...
            image=blanck(frame.height,frame.width,1)
            cv2.putText(image,str(temps)+' msec', (int(25), int(50)),cv2.FONT_HERSHEY_SIMPLEX, 1, 255,2, cv2.LINE_AA)
        imagecount.append(framecount)
        ...
        if k == 0x1b:
            ...
            print("Measured FPS", framecount/duration)
            ...
            l=len(imagecount)
            while j < l:
                file.write(str(imagecount[j])+' '+str(imagetemps[j])+'\n')
                j+=1
            file.close()
            break
    print("Finished")
